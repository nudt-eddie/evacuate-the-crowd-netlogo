...
to complex
  ...
  ask patches with [pxcor >= 42 and pxcor <= 47 and pycor >= 2 and pycor <= 20] [
    set pcolor violet
    set room "Wall2"
    ;set pcolor yellow
  ]
  ...
  ask patches with [pxcor >= -35 and pxcor <= -30 and pycor >= -25 and pycor <= -10] [
    set pcolor violet
    set room "Wall5"
    ;set pcolor yellow
  ]
...
to setup
  clear-all
  if simulateclass = "one-door"
  [
    onedoor
  ]
  if simulateclass = "one-door-obstacle"
  [
    onedoor
    oneobstract
  ]
  if simulateclass = "two-door"
  [
    twodoor
  ]
  if simulateclass = "two-door-corner"
  [
    twodoorcorner
  ]
  if simulateclass = "two-door-obstacle"
  [
    twodoor
    oneobstract
  ]
  if simulateclass = "two-door-corner-obstacle"
  [
    twodoorcorner
    oneobstract
  ]
  if simulateclass = "complex"
  [
    complex
  ]
...
;        setxy random-xcor random-ycor
        ...
        if simulateclass = "one-door" or simulateclass = "one-door-obstacle"
        [
          set target-door one-of patches with [room = one-of ["exit"] ]
        ]
        if simulateclass = "two-door" or simulateclass = "two-door-corner" or simulateclass = "two-door-obstacle"or simulateclass = "two-door-corner-obstacle"
        [
        ...
        [
          ...
          if target-door = 0 [
            set target-door one-of patches with [room = one-of ["exit1" "exit2" "exit3" "exit4" ] ]
          ]
...
to go
  ask turtles [
    ...
    if simulateclass = "complex"
    [
      if [room] of patch-here = "exit1" or [room] of patch-here = "exit2" or [room] of patch-here = "exit3"or [room] of patch-here = "exit4"[
...
to flee
  if simulateclass = "one-door-obstacle" or simulateclass = "two-door-obstacle" or simulateclass = "two-door-corner-obstacle"
  [
  ...
  if simulateclass = "one-door" or simulateclass = "two-door" or simulateclass = "two-door-corner"
  [
    set heading towards target-door + random-float 5
  ]
  if simulateclass = "complex"
  [
    ...
    panic
      set heading (towards min-one-of patches with [pcolor = violet ][distance myself] + 90 )
      if [pcolor] of patch-ahead 2 != black [
        fd 2
        ]
    ]
    ifelse count patches in-radius 5 with [pcolor = violet  ] > 0 [
      set heading (towards min-one-of patches with [pcolor = violet ][distance myself] + 90 + random-float 5 )
      if [pcolor] of patch-ahead 2 != black [
        fd 2
        ]
      ][
...
to fear
  ...
  if simulateclass = "two-door" or simulateclass = "two-door-corner" or simulateclass = "two-door-obstacle" or simulateclass = "two-door-corner-obstacle" or simulateclass = "complex" [
...
simulateclass
simulateclass
"one-door" "one-door-obstacle" "two-door" "two-door-corner" "two-door-obstacle" "two-door-corner-obstacle" "complex"
2
